...
def draw_graph(graph, agvs):
    ...
    with graph.lock:
        ...
        for node in G:
            if graph.cells[node].type in ["OPERATOR_STATION"]:
                    ...
                    positions_labes[node] = (positions[node][0], positions[node][1] - 0.2)
        ...
        for agv in agvs:
            ...
            positions_agv[agv.agv_id + 100] = (agv.x + agv.heading[0], agv.y + agv.heading[1])
    ...
    nx.draw_networkx_nodes(G2, pos=positions_agv, node_size=[0 if v >= 100 else 400 for v in positions_agv],node_shape="s", node_color = agv_color_map)
    ...
    nx.draw_networkx_edges(G2, pos=positions_agv, edgelist=heading_edges, edge_color='#420420', arrows=True)
